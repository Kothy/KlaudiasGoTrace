...
// order1007 merges a set of per-ParentId event batches into a single, consistent stream.
// The high level idea is as follows. Events within an individual batch are in
// correct order, because they are emitted by a single ParentId. So we need to produce
...
func order1007(m map[int][]*Event) (events []*Event, err error) {
	...
	// timestamp of event emission (ev.Timestmp).
	// We could not set ev.Timestmp = ev.args[2] earlier, because it would produce
	...
	for _, ev := range events {
		...
		case EvGoSysBlock, EvGoInSyscall:
			lastSysBlock[ev.GorutineId] = ev.Timestmp
		case EvGoSysExit:
			ts := int64(ev.args[2])
			if ts == 0 {
				continue
			}
			block := lastSysBlock[ev.GorutineId]
			if block == 0 {
...
func stateTransition(ev *Event) (g uint64, init, next gState) {
	...
	case EvGoStartLocal:
		...
		// frontier reaches it (EvGoStartLocal is emitted on the same ParentId
...
// order1005 merges a set of per-ParentId event batches into a single, consistent stream.
func order1005(m map[int][]*Event) (events []*Event, err error) {
	...
	for _, ev := range events {
		if ev.Typ == EvGoSysExit {
			// EvGoSysExit emission is delayed until the thread has a ParentId.
			// Give it the real sequence number and time stamp.
			ev.seq = int64(ev.args[1])
			if ev.args[2] != 0 {
				ev.Timestmp = int64(ev.args[2])
			}
		}
	}
...
func (l orderEventList) Less(i, j int) bool {
	return l[i].ev.Timestmp < l[j].ev.Timestmp
}
...
func (l eventList) Less(i, j int) bool {
	return l[i].Timestmp < l[j].Timestmp
}
